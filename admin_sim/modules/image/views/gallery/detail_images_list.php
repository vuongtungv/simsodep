...
function addTitleElement(titleElement,divNum,data) {
		$.ajax({
			url: "index2.php?module=gallery&view=gallery&raw=1&task=add_title_other_images",
			type: "get",
...
function removeElement(divNum,data) {
	  if (confirm('Bạn chắc chắn muốn xóa ảnh này?')){
		  ...
		  $.ajax({
				url: "index2.php?module=gallery&view=gallery&raw=1&task=delete_other_image",
				type: "get",
				data: "data="+data,
				error: function(){
	...
	$(function() {
		$("#sortable").sortable({
			update : function () {
				...
				$.ajax({
					url: "index2.php?module=gallery&view=gallery&raw=1&task=sort_other_images",
					type: "post",
					data: serial,
					error: function(){
