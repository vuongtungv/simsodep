	...
	class ExtendsModelsTables  extends FSModels 
	{
		...
		function table_new()
		{
			...
			if(!$this -> save_new_field($table_name))
				return;
			$row3 = array();	
			$row3['table_name'] = $table_name ;	
			$row3['name'] = $name ;	
			$row3['alias'] = $table_name ;	
			$row3['published'] = 1 ;	
			$this -> _add($row3, 'fs_extends');
		...
		function getTableFields()
		{
			...
			if($tablename == 'fs_extends')
				return;
			if(strpos($tablename, 'fs_extends_') === false){
...
//				$this -> createTable($tablename);
			...
			if($table_name_begin != $tablename){
				...
				$row3['alias'] = $table_name ;	
				$row3['published'] = 1 ;	
				$this -> _update($row3, 'fs_extends',' table_name = "'.$table_name_begin.'"');
			}
		...
		function remove()
		{
			...
			$query = " SELECT table_name FROM fs_extends
						WHERE id IN (".$str_cids.") ";
				global $db;
				$sql = $db->query($query);
				$tablenames = $db->getObjectList();
				if(!$tablenames)
					return;
			...
			if(count($tablenames))
			{
				...
				for($i = 0; $i < count($tablenames) ; $i ++){
					...
					$str_tablenames .= "'".$tablenames[$i] -> table_name."'";
					$str_tablenames_sort .= "".$tablenames[$i] -> table_name."";
				}
				
				// xóa trong bảng fs_extends_tables
				$sql = " DELETE FROM ".$this -> table_name."
						WHERE table_name IN ( $str_tablenames )  " ;
				$db->query($sql);
				$rows = $db->affected_rows();
				
				// xóa trong bảng fs_extends
				$sql = " DELETE FROM fs_extends
						WHERE table_name IN ( $str_tablenames )  " ;
				$db->query($sql);
				$rows = $db->affected_rows();
...
//				$rows = $db->affected_rows();
				...
				$sql_drop = " DROP TABLE  IF EXISTS  ".$str_tablenames_sort." ";
