<?php 
	class FaqModelsFaq extends FSModels
	{
		...
		function __construct()
		{
			...
			$this -> table_category_name = FSTable_ad::_('fs_faq_categories');
			$this -> table_types = FSTable_ad::_('fs_faq_types');
		...
		function save($row = array(), $use_mysql_real_escape_string = 1){
			...
			$cat =  $this->get_record_by_id($category_id,'fs_faq_categories');
	        	...
	        	foreach($field_change_arr as $field_item){
	        	      ...
	        	      if($field_value_original != $field_value_advice){
	        	          ...
	        	          if($field_item == 'category_id'){
	        	          		$cat =  $this->get_record_by_id($field_value_advice,'fs_news_categories');
								$row['category_id_wrapper'] = $cat -> list_parents;
								$row['category_alias_wrapper'] = $cat -> alias_wrapper;
								$row['category_name'] = $cat -> name;
								$row['category_alias'] = $cat -> alias;
