<?php
    class OrderModelsBanner extends FSModels
    {
        public $limit;
        public $prefix ;
        public function __construct()
        {
            $this -> limit = 40;
            $this -> view = 'banner';
            $this -> table_name = 'fs_order_2';
            $this -> table_name_item = 'fs_order_2_item';
            parent::__construct();
        }
...
//			}
            ...
            if (isset($_SESSION[$this -> prefix.'filter2'])) {
                ...
                if ($filter) {
                    $filter = (int)$filter - 1;
                    $where .= ' AND a.usage_status =  "'.$filter.'" ';
                }
            }
    ...
    //        $row['date_end'] = $end;
            ...
            if ($id && $row['status'] == 1) {
                //$order = $this->get_record_by_id($id, $this -> table_name);
                if ($order) {
                    //$setSubject = FSText::_('Thông tin xác nhận kích hoạt dịch vụ đơn hàng Đăng tuyển từ ketnoigiaoduc.vn').' - '.$order->code;
                    $list_item = $this->get_records(' published = 1 AND order_id = '.$id, $this -> table_name_item, '*');

                    $setSubject = $global_class->getConfig_email('subject_comfim_service_post');
                    $setSubject = str_replace('{code}', 'Banner -'.$order->code, $setSubject);

                    $body = $global_class->getConfig_email('email_published');
                    $body = str_replace('{username}', $order->name, $body);
                    $body = str_replace('{service}', 'Banner', $body);
                    $body = str_replace('{count}', count($list_item), $body);
                    ...
                    foreach ($list_item as $item) {
                        $service = $this->get_record_by_id($item->service_id, 'fs_banners_categories', 'name');
                        $html .= '
                                            <tr>
                                                <td style="border: 1px solid #ddd;">'.$i.'</td>
                                                <td style="border: 1px solid #ddd;"><h4 style="color:#006cc6;margin-bottom: 5px;text-align: left;">'.$service->name.'</h4></td>
                                                <td style="border: 1px solid #ddd;"><strong style="color: #e83e28;">'.$item->quantity.'</strong></td>
                                                <td style="border: 1px solid #ddd;"><strong style="color: #e83e28;">'.$item->discount.'</strong></td>
                                                <td style="border: 1px solid #ddd;"><strong style="color: #e83e28;">'.format_money($item->total_, ' VNĐ', '0').'</strong></td>
                                                <td style="border: 1px solid #ddd;"><strong style="color: #e83e28;">'.format_money($item->total_vat, ' VNĐ', '0').'</strong></td>
                                                <td style="border: 1px solid #ddd;"><strong style="color: #e83e28;">'.format_money($item->total_rs, ' VNĐ', '0').'</strong></td>
                                            </tr>
                                        ';
                        $i++;
                    }
