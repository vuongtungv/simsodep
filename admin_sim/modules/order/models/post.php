<?php
    class OrderModelsPost extends FSModels
    {
        public $limit;
        public $prefix ;
        public function __construct()
        {
            $this -> limit = 40;
            $this -> view = 'post';
            $this -> table_name = 'fs_order_post';
            $this -> table_name_item = 'fs_order_post_item';
            parent::__construct();
        }
    ...
    //        $row['date_end'] = $end;
            ...
            if ($id && $row['status'] == 1) {
                ...
                if ($order) {
                    ...
                    $list_item = $this->get_records(' published = 1 AND order_id = '.$id, 'fs_order_post_item', '*');

                    $setSubject = $global_class->getConfig_email('subject_comfim_service_post');
                    $setSubject = str_replace('{code}', 'Đăng tuyển -'.$order->code, $setSubject);
                    ...
                    foreach ($list_item as $item) {
                        $service = $this->get_record_by_id($item->service_id, 'fs_members_service', 'name');
                        $html .= '
                                            <tr>
