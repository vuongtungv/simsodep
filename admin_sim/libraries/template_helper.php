...
class TemplateHelper {
    ...
    public static function dt_date_pick($title, $name, $value, $default = '', $size = 60, $comment = '', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9', $class_col3 = 'col-md-6',$disabled =0) {
        ...
        echo '	</div>';
        if($disabled == 0){
            echo '      <a class="float-left fa fa-calendar" href="javascript:void(0)" onclick="javascript:NewCssCal (\'' . $name . '\',\'YYYYmmdd\',\'arrow\',true,\'24\')">';
        }else{
            echo '      <a class="float-left fa fa-calendar" disabled>';
        }
    ...
    public static function delete($link) {
        $link = "'".$link."'";
        $html='<a onclick="confirm_task('.$link.')" title="Xóa toàn bộ sim trong đại lý" data-toggle="modal" href="javascript:void(0)" class="btn btn-outline btn-circle btn-sm purple">
                                                            <i class="fa fa-minus-square"></i> Xóa </a>';
        return $html;
    }

    public static function allowed($link) {
        $link = "'".$link."'";
        $html='<a onclick="confirm_task('.$link.')" title="Duyệt toàn bộ sim trong đại lý" data-toggle="modal" href="javascript:void(0)" class="btn btn-outline btn-circle btn-sm purple">
    ...
    public static function genarate_form_liting($module, $view, $list, $fitler_config, $list_config, $sort_field, $sort_direct, $pagination = null, $params = array(), $first = 'index.php') {
                    ...
                    foreach ($col as $item) {
                        ...
                        switch ($type) {
                            ...
                            case 'edit':
                                ...
                                break;
                            case 'delete':
                                $link_delete = "index.php?module=" . $module . "&view=" . $view . "&task=delete&id=" . $row->id;
                                $html_body .= TemplateHelper::delete($link_delete);
                                break;
                            case 'allowed':
                                $link_allowed = "index.php?module=" . $module . "&view=" . $view . "&task=allowed&id=" . $row->id;
                                $html_body .= TemplateHelper::allowed($link_allowed);
                                break;
                            case 'view':
                            ...
                            case 'text_cod':
                                $code = $row->$item['field'];
                                if (is_numeric($code)){
                                    $text = '<a href="https://www.viettelpost.com.vn/Tracking?KEY='.$code.'">'.$code.'</a>';
                                }
                                else{
                                    $text = '<a href="http://www.vnpost.vn/vi-vn/dinh-vi/buu-pham?key='.$code.'">'.$code.'</a>';
                                }
                                $html_body .= $text;
                                break;
                            case 'text_order':
                                $code = $row->$item['field'];
                                if ($row->order_id){
                                    $text = '<a href="index.php?module=order&view=order&task=edit&id='.$row->order_id.'">'.$code.'</a>';
                                }
                                else{
                                    $text = $code.'<p>(đã xóa)<p>';
                                }
                                $html_body .= $text;
                                break;
                            case 'cno_sim':
                                ...
                                $html_body .= '<b style = "color:'.$color.'">'.$row->$item['field'].'</b></br>('.$row->agency_name.')';
                                break;
                            case 'admin_status':
                            ...
                            case 'status_order':
                                ...
                                $html_body .= '<b style = "color:'.$color.'">'.$row->$item['field'].'</b>';
                                break;
                            case 'phone_email_web':
                                $html_body .= $row->$item['field'];
                                $html_body .= "</br>".$row->email."";
                                $html_body .= "</br>".$row->web."";
                                break;
                            case 'cno_price':
                                $html_body .= format_money_0($row->$item['field'], '', '');
                                $html_body .= "</br>(".format_money_0($row->price_partner_end, '', '').")";
                                break;
                            case 'cno_commissions':
                                $html_body .= format_money_0($row->$item['field'], '', '').'</br>('.$row->commission_percent.'%)';
                                break;
                            case 'cno_partner':
                                $html_body .= format_money_0($row->$item['field'], '', '');
                                $html_body .= "</br>(".format_money_0($row->price_orginal, '', '').")";
                                break;
                            case 'cno_recive':
                                $html_body .= format_money_0($row->$item['field'], '', '');
                                $html_body .= "</br>(".format_money_0($row->price_revice, '', '').")";
                                break;
                            case 'cno_interest':
                                $html_body .= format_money_0($row->$item['field'], '', '');
                                $html_body .= "</br>(".format_money_0($row->price_support, '', '').")";
                                break;
                            case 'interest':
                                $interest = $row->price_public - $row->price + $row->commission_value;
                                $html_body .= format_money_0($interest);
                                break;
                            case 'price_receive':
                                $price_receive = $row->price - $row->commission_value;
                                $html_body .= format_money_0($price_receive);
                                break;
                            case 'commission':
                                $percent = $row->commission_value/$row->price * 100;
                                $html_body .= format_money_0($row->$item['field']).'('.$percent.'%)';
                                break;
                            case 'text_phone_order':
                                $arr_phone = explode(';', $row->$item['field']);
                                $t_phone = count($arr_phone);
                                if ($t_phone < 2) {
                                    $html_body .= $row->$item['field'];
                                }else{
                                    $html_body .= '<div class="op">';
                                    $html_body .= '<span class="one_p">'.$arr_phone[0].'...</span>';
                                    $html_body .= '<span class="all_p">'.$row->$item['field'].'</span>';
                                    $html_body .= '</div>';
                                }
                                break;
                            case 'edit_text':
                            ...
                            case 'note':
                                $sim = "'".$row->sim."'";
                                $estore_code = '<a data-toggle="modal" href="#small" class="btn btn-outline btn-circle btn-sm purple" onclick="NoteFunction('.$row->id.','.$sim.')">
                                                            <i class="fa fa-comment"></i> Note </a>';
                            ...
                            case 'order':
                                if ($row->status == 0) {
                                    ...
                                    $sim = "'".$row->sim."'";
                                    $price = "'".$row->price."'";
                                    $price1 = "'".format_money_0($row->price, '', '')."'";
                                    $commission_value = "'".$row->commission_value."'";
                                    $commission_value1 = "'".format_money_0($row->commission_value, '', '')."'";
                                    $price_public = "'".$row->price_public."'";
                                    $price_public1 = "'".format_money_0($row->price_public, '', '')."'";
                                    $estore_code = '<a data-toggle="modal" href="#large" class="btn btn-outline btn-circle btn-sm purple" onclick="OrderFunction('.$row->id.','.$sim.','.$price.','.$price1.','.$commission_value.','.$commission_value1.','.$price_public.','.$price_public1.')">
                                                                <i class="fa fa-cart-plus"></i> Đặt </a>';
                                    $html_body .= $estore_code;
                                }
                                break;
                            case 'status':
                                switch ($row->$item['field']) {
                                    case '0':
                                        $estore_code = '<span class="label label-sm label-warning"> Hiển thị </span>';
                                        break;
                                    case '1':
                                        $estore_code = '<span class="label label-sm label-info"> Giữ </span>';
                                        break;
                                    case '2':
                                        $estore_code = '<span class="label label-sm label-success"> Bán </span>';
                                        break;
                                    case '3':
                                        $estore_code = '<span class="label label-sm label-danger"> Ẩn </span>';
                                        break;
                                    default:
                                        break;
                                }
                            ...
                            case 'text_link':
                                ...
                                break;
                            case 'is_view':
                                $link_edit = "index.php?module=" . $module . "&view=" . $view . "&task=views&id=" . $row->id . "&page=" . $page;
                                $html_body .= TemplateHelper::edit($link_edit);
                                break;
                            case 'link_agency':
                                    $link = 'index.php?module=users&view=users&task=edit&id='.$row->agency;
                                    $html_body .= '<a style="text-align: left;" target="_blank" href="' . $link . '">' . $row->$item['field'] . '</a>';
                                break;
                            ...
                            case 'status':
                                ...
                                break;
                            case 'is_view':
                                $link_edit = "index.php?module=documents&view=document&task=edit&disable=1&id=" . $row->id . "&page=" . $page;
                                $html_body .= TemplateHelper::edit($link_edit);
                                break;
                            default:
        ...
        $html = $html_begin . $html_filter . $html_head . $html_body . $html_footer . '</form></div></div></div></div>';
...
//				$compare = $value;
    ...
    static function dt_edit_selectbox($title, $name, $value, $default = '', $arry_select = array(), $field_value = 'id', $field_label = 'name', $size = 1, $multi = 0, $add_fisrt_option = 0, $comment = '', $sub_item = '', $class = '', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9', $field = '') {
        // phan quyen theo field
        $module = FSInput::get('module');
        $view = FSInput::get('view', $module);
...
//				$compare = $value;
    ...
    static function dt_edit_text($title, $name, $value, $default = '', $maxlength = 255, $rows = 1, $editor = 0, $comment = '', $sub_item = '', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9',$disabled = 0) {
        ...
        if ($rows > 1) {
            if (!$editor) {
                echo '<textarea placeholder="" class="form-control" rows="' . $rows . '" cols="' . $size . '" name="' . $name . '" id="' . $name . '" >' . $value . '</textarea>';
            } else {
    ...
    public static function dt_edit_file($title, $name, $value = '', $comment = '', $class_col1 = 'col-md-3', $class_col2 = 'col-md-9') {
        ...
        $html ='
            ...
            <div class="form-group">
                <label class="control-label ' . $class_col1 . ' col-xs-12">' . $title . '</label>
                <div class="' . $class_col2 . '  col-xs-12">
                    <div class="fileinput fileinput-new" data-provides="fileinput">
                        <div class="input-group input-large">
                            ...
                            <span class="input-group-addon btn default btn-file">
                                ...
                                <input type="file" id ="'.$name.'" name="' . $name . '"> </span>
    ...
    public static function create_filter($row, $prefix) {
        ...
        if (isset($row['filter_count'])) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if ($type == 'yesno') {
                    ...
                    $html .= '          <div class="float-left pd-15 col-xs-2">';
                ...
                $html .= '          <div class="float-left pd-15 col-xs-2">';
                $html .= '              <select name="filter' . $i . '" class="form-control ' . $class . '"  onChange="this.form.submit()">';
                $html .= '                  <option value=""> -- ' . $filter_item['title'] . ' -- </option>';
        ...
        if (isset($row['search'])) {
            ...
            $html .= '	<div class="input-group custom-search-form">
                            <input type="text" placeholder="' . FSText :: _('Search') . '" name="keysearch" id="search" value="' . $ss_keysearch . '" class="form-control input-inline"  />
                        </span>
                        </div>			
                        ';
