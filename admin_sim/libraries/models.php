...
//			$this -> img_folder = '';
		...
		function get_sum($where = '',$table_name = '',$field = 'id'){
			if(!$where)
				return;
			if(!$table_name)
				...
				foreach($records as $item){
					...
					if ($this -> module == 'users' && $this -> view == 'price' && $new_record_id) {
						...
						SELECT 
							...
							created_time,
							commission_type
							FROM fs_price_commissions WHERE price_id = '.$item->id.';';
