<?php
	 ...
	 */
	// controller
class SignedsimControllersSignedsim extends FSControllers {
...
//			$parts=$model->get_parts_list();
			...
			// breadcrumbs
			$breadcrumbs = array ();
			$breadcrumbs [] = array (0 => FSText::_( 'Ký gửi sim' ), 1 => '' );
			global $tmpl;
			$tmpl->assign ( 'breadcrumbs', $breadcrumbs );
			
			$seo = $model->get_record('published = 1 AND module_seo = "deposit" ','fs_seo','title,keywords,description,content');
...
//			$sender_parts_name = $parts->name;
			...
			if($id)
			{
			    if($return){
			         ...
			         $link = FSRoute::_("index.php?module=signedsim&Itemid=14");
			    }
                 
				$msg = FSText::_("Bạn đã ký gửi sim thành công.");
...
//				}
				...
				echo "<script type='text/javascript'>alert('Xin lỗi bạn không gửi ký gửi sim được.'); </script>";
				$this -> display();
				return;
			}
		}
...
//			$sender_parts_name = $parts->name;
        ...
        if($id)
        {
            if($return){
                ...
                $link = FSRoute::_("index.php?module=home&view=home&task=success_kygui&Itemid=14");
            }
