		...
		function update_cat()
		{
			...
			$sql .= "
				...
				UPDATE fs_sim_".$item->id." SET cat_id = ',1',cat_alias = ',dau-so-co',cat_name = ',Sim đầu số cổ'  WHERE 
					LEFT(number, 4) IN (0903,0913,0983) AND cat_type IS NULL;
				...
				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',2,'),cat_alias = CONCAT(cat_alias, ',luc-quy,'),cat_name = CONCAT(cat_name, ',Sim lục quý,'),cat_type = '1'
				  	WHERE RIGHT(number, 6) IN (000000,111111,222222,333333,444444,555555,666666,777777,888888,999999) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',3,'),cat_alias = CONCAT(cat_alias, ',ngu-quy,'),cat_name = CONCAT(cat_name, ',Sim ngũ quý,'),cat_type = '1'
					WHERE RIGHT(number, 5) IN (00000,11111,22222,33333,44444,55555,66666,77777,88888,99999) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',4,'),cat_alias = CONCAT(cat_alias, ',tu-quy,'),cat_name = CONCAT(cat_name, ',Sim tứ quý,'),cat_type = '1' 
					WHERE RIGHT(number, 4) IN (0000,1111,2222,3333,4444,5555,6666,7777,8888,9999) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',5'),cat_alias = CONCAT(cat_alias, ',tam-hoa-kep'),cat_name = CONCAT(cat_name, ',Sim tam hoa kép'),cat_type = '1'
					WHERE RIGHT(number, 3) IN (000,111,222,333,444,555,666,777,888,999) AND SUBSTRING(number, 5, 3) IN (000,111,222,333,444,555,666,777,888,999) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',6'),cat_alias = CONCAT(cat_alias, ',tam-hoa'),cat_name = CONCAT(cat_name, ',Sim tam hoa')
					WHERE RIGHT(number, 3) IN (000,111,222,333,444,555,666,777,888,999) AND SUBSTRING(number, 6, 1) != SUBSTRING(number, 7, 1) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',7'),cat_alias = CONCAT(cat_alias, ',taxi-2'),cat_name = CONCAT(cat_name, ',Sim taxi 2'),cat_type = '1'
					WHERE RIGHT(number, 2) = SUBSTRING(number, 7, 2) AND SUBSTRING(number, 7, 2) = SUBSTRING(number, 5, 2) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',8,9'),cat_alias = CONCAT(cat_alias, ',taxi-3,taxi-aba-aba'),cat_name = CONCAT(cat_name, ',Sim taxi 3,Sim taxi ABA.ABA'),cat_type = '1'
					WHERE RIGHT(number, 3) = SUBSTRING(number, 5, 3) AND SUBSTRING(number, 10, 1) = SUBSTRING(number, 8, 1) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',8,10'),cat_alias = CONCAT(cat_alias, ',taxi-3,taxi-aab-aab'),cat_name = CONCAT(cat_name, ',Sim taxi 3,Sim taxi AAB.AAB'),cat_type = '1'
					WHERE RIGHT(number, 3) = SUBSTRING(number, 5, 3) AND SUBSTRING(number, 8, 1) = SUBSTRING(number, 9, 1) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',8,11'),cat_alias = CONCAT(cat_alias, ',taxi-3,taxi-baa-baa'),cat_name = CONCAT(cat_name, ',Sim taxi 3,Sim taxi BAA.BAA'),cat_type = '1'
					WHERE RIGHT(number, 3) = SUBSTRING(number, 5, 3) AND SUBSTRING(number, 10, 1) = SUBSTRING(number, 9, 1) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',8,12'),cat_alias = CONCAT(cat_alias, ',taxi-3,taxi-abc-abc'),cat_name = CONCAT(cat_name, ',Sim taxi 3,Sim taxi ABC.ABC'),cat_type = '1'
					WHERE RIGHT(number, 3) = SUBSTRING(number, 5, 3) AND SUBSTRING(number, 10, 1) != SUBSTRING(number, 9, 1) AND SUBSTRING(number, 8, 1) != SUBSTRING(number, 9, 1)  AND cat_type IS NULL;
				
				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',13'),cat_alias = CONCAT(cat_alias, ',taxi-4'),cat_name = CONCAT(cat_name, ',Sim taxi 4')
					WHERE RIGHT(number, 4) = SUBSTRING(number, 3, 4) AND cat_name = '' AND cat_type IS NULL;
				...
				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',39,40'),cat_alias = CONCAT(cat_alias, ',so-ganh,g-aba-cdc'),cat_name = CONCAT(cat_name, ',Sim số gánh,Sim số gánh ABA.CDC')  WHERE 
					RIGHT(number, 1) = SUBSTRING(number, 8, 1) AND SUBSTRING(number, 5, 1) = SUBSTRING(number, 7, 1) AND cat_type IS NULL;

				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',39,41'),cat_alias = CONCAT(cat_alias, ',so-ganh,g-aba'),cat_name = CONCAT(cat_name, ',Sim số gánh,Sim số gánh ABA')  WHERE 
				...
				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',42,43'),cat_alias = CONCAT(cat_alias, ',ganh-dao,g-abba-cddc'),cat_name = CONCAT(cat_name, ',Sim gánh đảo,Sim gánh đảo ABBA.CDDC')  WHERE 
				...
				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',42,44'),cat_alias = CONCAT(cat_alias, ',ganh-dao,g-abba'),cat_name = CONCAT(cat_name, ',Sim gánh đảo,Sim gánh đảo ABBA')  WHERE 
				...
				Update fs_sim_".$item->id." SET cat_id = CONCAT(cat_id, ',46,48'),cat_alias = CONCAT(cat_alias, ',tien-doi,tien-doi-2'),cat_name = CONCAT(cat_name, ',Sim tiến đôi,Sim tiến 2 đôi cuối')  WHERE 
					RIGHT(number, 1) - SUBSTRING(number, 8, 1) = 1 AND SUBSTRING(number, 9, 1) = SUBSTRING(number, 7, 1) AND cat_type IS NULL;
