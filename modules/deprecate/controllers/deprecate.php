<?php
	 ...
	 */
	// controller
class DeprecateControllersDeprecate extends FSControllers {
...
//			$parts=$model->get_parts_list();
			...
			// breadcrumbs
			$breadcrumbs = array ();
			$breadcrumbs [] = array (0 => FSText::_( 'Tìm sim theo yêu cầu' ), 1 => '' );
			global $tmpl;
			$tmpl->assign ( 'breadcrumbs', $breadcrumbs );

			$seo = $model->get_record('published = 1 AND module_seo = "find" ','fs_seo','title,keywords,description,content');
...
//			$parts = $model->get_record(' email = "'.$sender_parts.'"','fs_deprecate_parts');
//			$sender_parts_name = $parts->name;
			...
			if($id)
			{
			    if($return){
			         ...
			         $link = FSRoute::_("index.php?module=deprecate&Itemid=14");
			    }
...
//			$sender_parts_name = $parts->name;
			...
			if($id)
			{
			    if($return){
			         ...
			         $link = FSRoute::_("index.php?module=home&view=home&task=success_timsim&Itemid=14");
			    }

				$msg = FSText::_("Gửi thông tin tìm sim thành công.");
//				if(!$this -> send_mail($sender_name,$sender_email,$sender_parts_name)){
//					$msg = FSText::_("Cám ơn bạn đã liên lạc với chúng tôi.");
//				}
				setRedirect($link);
				...
				echo "<script type='text/javascript'>alert('Xin lỗi bạn không gửi được thông điệp tìm sim.'); </script>";
				$this -> display();
				return;
			}
		}
